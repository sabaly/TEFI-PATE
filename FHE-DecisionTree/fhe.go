...
func EvalSign(ct *rlwe.Ciphertext, pk *rlwe.PublicKey, params hefloat.Parameters, eval *hefloat.Evaluator) (res *rlwe.Ciphertext) {
	prec := params.EncodingPrecision()

	Sign := func(x float64) (y float64) {
		if x <= 0 {
			y = 1
		} else {
			y = 0
	...
	interval := bignum.Interval{
		Nodes: 15,
		A:     *bignum.NewFloat(-1000000, prec),
		B:     *bignum.NewFloat(1000000, prec),
	}

	poly := bignum.ChebyshevApproximation(Sign, interval)
...
func EvalSign_bis(ct *rlwe.Ciphertext, pk *rlwe.PublicKey, params hefloat.Parameters, eval *hefloat.Evaluator, deep float64) (res *rlwe.Ciphertext) {
	prec := params.EncodingPrecision()
